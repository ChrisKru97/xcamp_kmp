---
name: mobile-design
description: Mobile-first design thinking and decision-making for iOS and Android apps. Touch interaction, performance patterns, platform conventions. Teaches principles, not fixed values. Use when building Kotlin Multiplatform, SwiftUI, Jetpack Compose, or native mobile apps.
...
| **Framework** | "Kotlin Multiplatform, SwiftUI, Compose, or native?" | Determines patterns and tools |
| **Navigation** | "Tab bar, drawer, or stack-based?" | Core UX decision |
| **State** | "What state management? (ViewModel/Flow/StateFlow/Combine?)" | Architecture foundation |
...
|-------------|----------------|--------------|
| **ScrollView/LazyColumn for long lists** | Renders ALL items, memory explodes | Use `LazyColumn`/`LazyVStack` with proper keys |
| **Missing stable keys** | Wrong item recycling, bugs on reorder | Stable, unique ID from data |
| **Rebuild entire view on state change** | Unnecessary recomposition, janky UI | Targeted state updates, `derivedStateOf` |
| **Global state for everything** | Unnecessary recompositions | Local state default, lift when needed |
| **Blocking main thread** | Frozen UI, janky scroll | Offload work to Dispatchers/IO |
| **Excessive recomposition** | Battery drain, slow UI | `remember`, `stable` props, memoization |
| **console.log/print in production** | Performance degradation | Remove before release build |
| **Skip view reuse** | Memory leaks, slow scroll | Proper view holders / view reuse |
...
| ❌ NEVER DO | Why It's Wrong | ✅ ALWAYS DO |
|-------------|----------------|--------------|
| **Token in plain storage** | Easily accessible, stolen on rooted device | `Keychain` (iOS) / `EncryptedSharedPreferences` (Android) |
| **Hardcode API keys** | Reverse engineered from APK/IPA | BuildConfig, secure storage, backend proxy |
...
Framework:  Kotlin Multiplatform (shared logic) + SwiftUI (iOS) + Compose (Android)
Files Read: touch-psychology.md, mobile-performance.md, platform-ios.md, platform-android.md

3 Principles I Will Apply:
1. LazyColumn/LazyVStack with stable keys for all lists
2. 44-48px touch targets, thumb zone for primary CTAs
3. Platform-specific navigation (edge swipe iOS, back button Android)

Anti-Patterns I Will Avoid:
1. ScrollView/Column for lists → LazyColumn/LazyVStack
2. Unstable keys → Stable IDs from data
3. Plain storage for tokens → Keychain/EncryptedSharedPreferences
```
...
WHAT ARE YOU BUILDING?
        ...
        │       ├── Share: business logic, data layer, repositories
        │       ├── Platform-specific: SwiftUI (iOS), Compose (Android)
        │       └── Best for: native UI with shared backend
        │
        ├── Maximum native performance + single platform
        │   ├── iOS only → SwiftUI
        │   └── Android only → Kotlin + Jetpack Compose
        │
        ├── Deep native features + platform-specific optimizations
        │   └── Native development (SwiftUI or Compose)
        │
        └── Need OTA updates + rapid iteration
            └── Consider React Native + Expo (if applicable)
```
...
- [ ] **Framework chosen?** (KMP / Native SwiftUI / Native Compose)
- [ ] **Navigation pattern decided?** (Tabs / Stack / Drawer)
- [ ] **State management selected?** (ViewModel/Flow/StateFlow for Android, Observable/Combine for iOS)
...
- [ ] **SSL pinning enabled?**
- [ ] **Lists optimized (stable keys, proper view reuse)?**
- [ ] **Memory cleanup on dispose/viewModel clear?**
